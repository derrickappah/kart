 ...
 * @param {Object} [options] - Additional options (e.g. type, tag, url, icon, avatarUrl, image, data)
...
export async function triggerPushNotification(userId, title, message, relatedOrderId = null, options = {}) {
  ...
  try {
    ...
    const notificationIcon = options.icon || options.avatarUrl || '/icon.png';
    const notificationImage = options.image || undefined;
      ...
      if (!vapidPublicKey || !vapidPrivateKey) {
        ...
        const payload = JSON.stringify({
          ...
          data: {
            ...
            avatarUrl: options.avatarUrl || (notificationIcon !== '/icon.png' ? notificationIcon : null),
          ...
          try {
            await messaging.send({
              ...
              notification: {
                title,
                body: message,
                ...(notificationIcon && notificationIcon.startsWith('http') ? { imageUrl: notificationIcon } : {})
              },
              data: {
                url: targetUrl,
                type: notificationType,
                avatarUrl: options.avatarUrl || notificationIcon || '',
              ...
              apns: {
                ...
                fcm_options: {
                  ...(notificationIcon && notificationIcon.startsWith('http') ? { image: notificationIcon } : {})
                }
              }
            });
            result.delivered += 1;
 ...
 * @param {Object} params - { userId, type, title, message, relatedOrderId, options, data }
 * @returns {Promise<Object>} Created notification
 */
export async function createNotification(supabase, { userId, type, title, message, relatedOrderId = null, options = {}, data: extraData = null }) {
  const insertPayload = {
    user_id: userId,
    type,
    title,
  ...
  // Fire push notification asynchronously with complete options (including profile avatar)
  triggerPushNotification(userId, title, message, relatedOrderId, {
    type,
    ...(options || {}),
    data: {
      ...(options?.data || {}),
...
export async function createNotifications(supabase, notifications) {
  ...
  notifications.forEach(n => {
    triggerPushNotification(n.userId, n.title, n.message, n.relatedOrderId, {
      type: n.type,
      ...(n.options || {})
    }).catch(err => {
      console.error('[Notifications] Failed to trigger push notification:', err.message);
