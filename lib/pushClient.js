...
export async function registerForPushNotifications(options = {}) {
  ...
  try {
    ...
    if (permission !== 'granted') {
      return { success: false, error: 'Push notification permission was denied in browser settings' };
    }
    ...
    // Remove any previous stale subscription before renewing with current VAPID key
    try {
      ...
      console.warn('[PushClient] Subscription refresh warning:', cleanErr);
    }
